<script>
	import { format, geoEquirectangular, geoGraticule10, geoPath, scaleSqrt } from "d3";
	...
	const selectorPoints = [...points].sort((a, b) => a.country.localeCompare(b.country));
	...
	function showTooltip(event, datum) {
		...
		const x = Number.isFinite(event.clientX) ? event.clientX - bounds.left : target.left + target.width / 2 - bounds.left;
		const y = Number.isFinite(event.clientY) ? event.clientY - bounds.top : target.top - bounds.top;
		tooltip = {
			datum,
...
<figure class="bubble-map">
	<figcaption>
		...
		<p>Bubble area represents the number of distinct years with recorded disaster displacement from 2008 to 2025. Choose a country, or hover or tap a bubble, for details.</p>
	</figcaption>

	<div class="country-selector" aria-label="Select a Pacific country or territory">
		<strong>Country or territory</strong>
		<button class:active={!selected} type="button" aria-pressed={!selected} onclick={resetMap}>All</button>
		{#each selectorPoints as datum}
			<button
				class:active={selected?.country === datum.country}
				type="button"
				aria-pressed={selected?.country === datum.country}
				onclick={() => selectCountry(datum)}
			>{datum.country}</button>
		{/each}
	</div>

	<div class="map-wrap" bind:this={container} role="group" aria-label="Interactive Pacific displacement bubble map" onpointerleave={() => { if (!selected) tooltip = null; }}>
		...
		<svg viewBox={`0 0 ${width} ${height}`} role="img" aria-labelledby="pacific-map-title pacific-map-desc" onclick={resetMap}>
			<title id="pacific-map-title">Reported disaster displacement events across Pacific Island countries and territories</title>
			<desc id="pacific-map-desc">A bubble map of 17 Pacific Island countries and territories. Papua New Guinea has the most years affected with 15, followed by Fiji with 14 and Solomon Islands with 13.</desc>
			...
			{#each points as datum}
				<circle
					...
					onblur={() => { if (!selected) tooltip = null; }}
					onclick={(event) => {
			...
			<g class="size-key" transform="translate(38,447)" aria-hidden="true">
				...
				{#each sizeKey as item}
					...
					<text class="key-value" x={item.x} y="100" text-anchor="middle">{item.value}</text>
				{/each}
			</g>
		</svg>
		...
		{#if tooltip}
			<div class:below={tooltip.below} class="tooltip" style={`left:${tooltip.x}px;top:${tooltip.y}px`} role="group" aria-label={`${tooltip.datum.country} displacement details`}>
				...
				<dl>
					...
					<div><dt>Internal displacements</dt><dd>{number(tooltip.datum.total_internal_displacements)}</dd></div>
					<div><dt>Recorded period</dt><dd>{tooltip.datum.first_year}–{tooltip.datum.latest_year}</dd></div>
				</dl>
			</div>
		{/if}
	</div>

	<p class="note">Each calendar year is counted once, even when multiple displacement events were reported during that year. Bubbles mark countries and territories, not individual event locations. “Internal displacements” counts movements rather than unique people.</p>
	<p class="source">Source: <a href="https://www.internal-displacement.org/database/displacement-data/" target="_blank" rel="noreferrer">Internal Displacement Monitoring Centre (IDMC)</a>. Basemap: <a href="https://www.naturalearthdata.com/downloads/50m-cultural-vectors/50m-admin-0-countries-2/" target="_blank" rel="noreferrer">Natural Earth</a>.</p>
</figure>

<style>
	.bubble-map { width: min(100%,1080px); margin: clamp(64px,8vw,104px) auto clamp(42px,5vw,68px); font-family: "Inter",sans-serif; }
	figcaption { max-width: 900px; }
	h5 { margin: 0; font: 700 clamp(18px,2vw,24px)/1.2 "Inter",sans-serif; letter-spacing: -.02em; }
	figcaption p { margin: 8px 0 22px; color: #444; font: 400 clamp(15px,1.6vw,19px)/1.45 "Inter",sans-serif; }
	.country-selector { display: flex; flex-wrap: wrap; align-items: center; gap: 7px 8px; margin: 0 0 24px; }
	.country-selector strong { margin-right: 4px; font: 700 13px/1 "Inter",sans-serif; text-transform: uppercase; letter-spacing: .06em; }
	.country-selector button { display: inline-flex; align-items: center; gap: 6px; min-height: 32px; padding: 5px 8px; border: 1px solid #ccc; border-radius: 3px; color: #555; background: #fff; font: 500 13px/1.2 "Inter",sans-serif; cursor: pointer; }
	.country-selector button.active { color: #111; border-color: #111; }
	.country-selector button:focus-visible { outline: 2px solid #135ae1; outline-offset: 2px; }
	.map-wrap { position: relative; width: 100%; }
	svg { display: block; width: 100%; height: auto; overflow: hidden; }
	.ocean { fill: #2197ff; fill-opacity: .12; }
	.graticule { fill: none; stroke: #2197ff; stroke-opacity: .13; stroke-width: .8; }
	.world-land path { fill: #a7abaf; fill-opacity: .24; stroke: #808080; stroke-opacity: .28; stroke-width: .55; vector-effect: non-scaling-stroke; }
	.pacific-land path { fill: #a7abaf; fill-opacity: .58; stroke: #808080; stroke-opacity: .52; stroke-width: .7; vector-effect: non-scaling-stroke; }
	circle[role="button"] { fill: #135ae1; fill-opacity: .82; cursor: pointer; stroke: #135ae1; stroke-width: 1.5; transition: opacity 150ms ease,stroke-width 100ms ease,fill-opacity 100ms ease; }
	circle[role="button"]:hover,circle[role="button"]:focus,.selected { outline: none; stroke: #135ae1; stroke-width: 4; }
	circle[role="button"]:hover,circle[role="button"]:focus,.selected { fill-opacity: 1; }
	circle.dimmed { opacity: .3; }
	...
	@media(max-width:700px){
		.bubble-map { width: calc(100% + 40px); margin-left: -20px; }
		figcaption,.country-selector,.note,.source { padding-inline: 20px; }
		figcaption p { margin-bottom: 14px; }
		.country-selector { gap: 6px; }
		.country-selector button { font-size: 12px; }
		.size-key { transform: translate(28px,447px); }
	}
</style>
