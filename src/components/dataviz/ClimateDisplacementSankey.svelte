<script>
	...
	function group(name) {
		...
		if (name.includes("Mass Movement") || name.includes("Landslide"))
			return "mass";
	...
	function label(name) {
		if (name.startsWith("Climate-driven"))
			return "People affected by disasters";
	...
	function valueLabel(node) {
		...
		return node.value >= 5_000
			? `${format(Math.round(node.value))} movements`
			: "";
	}
			...
			for (const word of words) {
				...
				if (tspan.node().getComputedTextLength() > width && line.length > 1) {
					...
					tspan = text
						...
						.attr("x", x)
						.attr("y", y)
						.attr("dy", `${++lineNumber * 1.16}em`)
						.text(word);
				}
			}
			this.__data__.labelLines = text.selectAll("tspan").size();
		});
	}
	...
	function showTooltip(event, html) {
		...
		tooltipX =
			...
			Math.max(118, Math.min(bounds.width - 118, event.clientX - bounds.left));
		tooltipY = Math.max(92, event.clientY - bounds.top);
		tooltipVisible = true;
	}

	function render() {
		const viewportWidth = Math.max(320, container.clientWidth);
		const compact = viewportWidth < 720;
		const width = compact ? 860 : viewportWidth;
		const height = compact ? 850 : 720;
		const margin = compact
			? { top: 58, right: 100, bottom: 28, left: 100 }
		...
		const columns = [
			...
			[
				"Hazard family",
				(d) =>
					d.name.endsWith("disasters") && !d.name.startsWith("Climate-driven")
			],
			["Hazard type", (d) => d.name.startsWith("Hazard type:")],
			["Hazard subtype", (d) => d.name.startsWith("Hazard subtype:")]
		].map(([text, match]) => ({
			text,
			x: d3.mean(graph.nodes.filter(match), (d) => d.x0)
		}));
		...
		const link = chart
			...
			.attr("stroke", (d) => colors[group(d.target.name)])
		...
		node
			...
			.attr("stroke", (d) => d3.color(colors[d.group]).darker(0.45));

		function labelX(d) {
			return d.x0 < width / 2 ? d.x0 - 8 : d.x1 + 8;
		}
		function anchor(d) {
		...
		node
			...
			.attr(
				...
				(d) =>
					(d.y0 + d.y1) / 2 +
					17 +
					Math.max(0, (d.labelLines ?? 1) - 1) * (compact ? 11 : 13)
			)
			.attr("text-anchor", anchor)
			.text(valueLabel);
		...
		function applyStep() {
			...
			const activeLink = graph.links.find(
				(d) => d.source.name === step.source && d.target.name === step.target
			);
			if (!activeLink) return;
			link.attr("stroke-opacity", (d) => (d === activeLink ? 0.82 : 0.075));
			node.attr("opacity", (d) =>
				d === activeLink.source || d === activeLink.target ? 1 : 0.16
			);
		}
		...
		panToStep = () => {
			if (!compact) return;
			const positions = [0, 310, 455, 455, 455];
			const requested = positions[activeStep ?? 0] ?? 0;
			const maxScroll = Math.max(0, width - container.clientWidth);
			container.scrollTo({
				left: Math.min(requested, maxScroll),
				behavior: "smooth"
			});
		};
		applyStep();
		...
		link
			.on("pointerenter", function (event, d) {
				...
				node.attr("opacity", (item) =>
					item === d.source || item === d.target ? 1 : 0.2
				);
				showTooltip(
					event,
					`<strong>${label(d.source.name)} → ${label(d.target.name)}</strong><span>${format(d.value)} displacement movements</span>`
				);
			})
		...
		node
			.on("pointerenter", function (event, d) {
				...
				link.attr("stroke-opacity", (item) =>
					related.has(item) ? 0.72 : 0.06
				);
				node.attr("opacity", (item) =>
					item === d ||
					related.has(item.sourceLinks?.[0]) ||
					related.has(item.targetLinks?.[0])
						? 1
						: 0.24
				);
				showTooltip(
					event,
					`<strong>${label(d.name)}</strong><span>${format(Math.round(d.value))} displacement movements</span>`
				);
			})
			.on("pointermove", (event, d) =>
				showTooltip(
...
<figure class="sankey-figure">
	<figcaption>
		...
		<p class="dek">
			Follow 907,531 recorded internal displacement movements through hazard
			families, types, and subtypes. Scroll to explore.
		</p>
	</figcaption>
	<div class="scrolly">
		<div class="chart" bind:this={container}>
			<svg
				bind:this={svg}
				role="img"
				aria-label="Sankey diagram tracing Pacific disaster displacement movements from disaster impact through hazard families, types, and subtypes"
			></svg>
			<p class="mobile-pan-hint" aria-hidden="true">
				Swipe to follow the flow <span>→</span>
			</p>
			<div
				bind:this={tooltip}
		...
		<div class="steps">
			<Scrolly bind:value={activeStep} top={120} bottom={120}>
				...
				>
					<div class="step-card">
						<p>
							Between 2018 and 2025, storms caused significantly more flight
							than floods or earthquakes.
						</p>
					</div>
				</section>
				<section
				...
				>
					<div class="step-card">
						<p>
							Within this category, typhoons, hurricanes, and cyclones form the
							leading hazard subtype for displacements. Tropical cyclones
							dominate the flow through nearly every country in the region.
						</p>
					</div>
				</section>
				<section
				...
				>
					<div class="step-card">
						<p>
							A few catastrophic cyclones account for a disproportionate share
							of this total, most notably Cyclone Winston in Fiji in 2016.
						</p>
					</div>
				</section>
				<section
				...
				>
					<div class="step-card">
						<p>
							Floods, the second-leading driver, caused 227,226 movements across
							71 events. They leave a lingering aftermath of contaminated
							freshwater supplies, destroyed crops, and waterlogged soils.
						</p>
					</div>
				</section>
			</Scrolly>
	...
	<p class="note">
		The disaster affected-population total and displacement total measure
		different things: displacement data count movements, not unique people.
	</p>
	<p class="source">
...
<style>
	.sankey-figure {
		width: min(1400px, calc(100vw - clamp(32px, 6vw, 96px)));
		max-width: none;
		margin: clamp(64px, 8vw, 104px) 0 clamp(42px, 5vw, 68px);
		margin-left: 50%;
	...
	.step-card p {
		...
		text-align: center;
		font:
			500 clamp(17px, 1.7vw, 23px)/1.48 "Inter",
			sans-serif;
